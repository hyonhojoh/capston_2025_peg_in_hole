...
class ManagerBasedRLEnv(ManagerBasedEnv, gym.Env):
    ...
    def __init__(self, cfg: ManagerBasedRLEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        self.shared_dict["rewards"] = {}
        self.shared_dict["observations"] = {}
        self.shared_dict["terminations"] = {}
        ...
        self.data_log = []
        self.output_csv_path = "/home/hyunho_RCI/IsaacLab/source/isaaclab_tasks/isaaclab_tasks/manager_based/rci/insertion-v1/test/data/data_3.csv"
    ...
    def step(self, action: torch.Tensor) -> VecEnvStepReturn:
        ...
        # # -----------------------------------------------------------------------
        # # (B) CSV 저장에 필요한 데이터 수집 및 로그
        # # -----------------------------------------------------------------------
        # # 예) "policy" 그룹의 관측만 저장한다고 가정
        # if "policy" in self.obs_buf:
        #     obs_policy_envs = self.obs_buf["policy"].cpu().numpy()  # shape (num_envs, obs_dim)
        # else:
        #     # 만약 policy 그룹이 없으면 빈 array
        #     obs_policy_envs = np.zeros((self.num_envs, 0))

        # if "critic" in self.obs_buf:
        #     obs_critic_envs = self.obs_buf["critic"].cpu().numpy()  # shape (num_envs, obs_dim)
        # else:
        #     # 만약 policy 그룹이 없으면 빈 array
        #     obs_critic_envs = np.zeros((self.num_envs, 0))

        # actions_envs = action.cpu().numpy()  # shape (num_envs, action_dim)
        # rewards_envs = self.reward_buf.cpu().numpy()  # shape (num_envs,)

        # # num_envs만큼 row 생성
        # for env_id in range(self.num_envs):
        #     row = np.concatenate((
        #         [env_id],             # 어떤 env인지
        #         obs_policy_envs[env_id],
        #         obs_critic_envs[env_id],
        #         actions_envs[env_id],
        #         [rewards_envs[env_id]]
        #     ))
        ...
        # # 예) 에피소드가 끝난 env가 있다면 로그 저장(원하는 시점에 맞춰 호출 가능)
        # # 여기서는 첫 번째 환경이 리셋되었을 때라고 가정
        # # (원하면 "if len(reset_env_ids) > 0:" 내부에 넣거나, 원하는 조건으로 변경하세요)
        # if len(reset_env_ids) > 0:
        #     # 에피소드가 끝날 때마다 저장, 혹은 누적된 데이터를 통으로 저장
    ...
    def save_data_to_csv(self):
        """데이터 로그(self.data_log)를 CSV 파일로 저장."""
        if not self.data_log:
            print("[INFO] No data to save yet.")
            return

        # 아래는 예시로, obs_dim=10, action_dim=7 정도로 가정
        # 실제 obs_dim, action_dim은 사용자가 알고 있는 값에 맞춰 설정해주세요.
        policy_dim = self.obs_buf["policy"].shape[1] if "policy" in self.obs_buf else 0
        critic_dim = self.obs_buf["critic"].shape[1] if "critic" in self.obs_buf else 0
        action_dim = sum(self.action_manager.action_term_dim)

        # 열 이름 예시(env_id, obs, critic, action, reward)
        columns = (
            ["env_id"]
            + [f"obs_{i}" for i in range(policy_dim)]
            + [f"critic_{j}" for j in range(critic_dim)]
            + [f"action_{k}" for k in range(action_dim)]
            + ["reward"]
        )
        ...
        print(f"[INFO] CSV log saved to: {self.output_csv_path}")
