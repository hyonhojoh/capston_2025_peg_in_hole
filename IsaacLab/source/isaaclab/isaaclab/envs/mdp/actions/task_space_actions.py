...
from isaaclab.controllers.task_space_impedance import TaskSpaceImpedanceController
...
class TaskSpaceImpedanceAction(ActionTerm):
    ...
    cfg: actions_cfg.TaskSpaceImpedanceControllerActionCfg
    """The configuration of the action term."""
    _asset: Articulation
    _scale: torch.Tensor

    def __init__(self, cfg: actions_cfg.TaskSpaceImpedanceControllerActionCfg, env: ManagerBasedEnv):
        # initialize the action term
        super().__init__(cfg, env)
        ...
        self._controller = TaskSpaceImpedanceController(
            cfg=self.cfg.controller, num_envs=self.num_envs, num_dof=self._num_DoF, device=self.device
        )

        self._raw_actions = torch.zeros(self.num_envs, self.action_dim, device=self.device)
        self._processed_actions = torch.zeros(self.num_envs, 7, device=self.device) # pos + quat
        ...
        # save the scale as tensors
        self._scale = torch.zeros((self.num_envs, 6), device=self.device)
        self._scale[:] = torch.tensor(self.cfg.scale, device=self.device)
    ...
    def set_gain(self, stiffness: list, damping_ratio: list) -> None:
        self._controller._p_gains[:] = torch.tensor(stiffness, device=self.device)
        self._controller._d_gains[:] = 2 * torch.sqrt(self._controller._p_gains) * torch.tensor(damping_ratio, device=self.device)
    ...
    def process_actions(self, actions: torch.Tensor):
        ...
        actions = (
            ...
            + (1.0 - self.cfg.ema_factor) * self._raw_actions.clone().to(self.device)
        )
        # store the raw actions
        self._raw_actions[:] = actions
        ...
        if self.cfg.controller.is_restricted:
            pos_actions = actions[:, 0:3] * self._scale[:, 0:3]
            rot_actions = torch.zeros(self.num_envs, 3, device=self.device)
            rot_actions[:, 2] = actions[:, 3] * self._scale[:, 5] # (0, 0, dyaw)
        else:
            pos_actions = actions[:, 0:3] * self._scale[:, 0:3]
            rot_actions = actions[:, 3:6] * self._scale[:, 3:6]

        ee_pos_des, ee_quat_des = math_utils.apply_delta_pose(
            self._ee_pose_b[:, 0:3], self._ee_pose_b[:, 3:7], torch.cat((pos_actions, rot_actions), dim=1)
        )
        
        if self.cfg.controller.is_restricted:
            target_euler_xyz = torch.stack(math_utils.euler_xyz_from_quat(ee_quat_des), dim=1)
            target_euler_xyz[:, 0] = np.pi
            target_euler_xyz[:, 1] = 0.0

            ee_quat_des = math_utils.quat_from_euler_xyz(
                target_euler_xyz[:, 0], target_euler_xyz[:, 1], target_euler_xyz[:, 2]
            )

        self._processed_actions[:, 0:3] = ee_pos_des
        self._processed_actions[:, 3:7] = ee_quat_des

        self._controller.set_command(
            command=self._processed_actions,
        )
    ...
    def apply_actions(self):
        ...
        dof_target, _ = self._controller.compute(
            joint_pos=self._joint_pos,
            joint_vel=self._joint_vel,
            current_ee_pose_b=self._ee_pose_b,
            current_ee_vel_b=self._ee_vel_b,
            jacobian_b=self._jacobian_b,
            arm_mass_matrix=self._mass_matrix,
            gravity=self._gravity
        )
        # set the joint position command
        self._asset.set_joint_effort_target(dof_target, self._joint_ids)
